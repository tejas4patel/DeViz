...
export default function Scene08Maternal() {
  ...
  useEffect(() => {
    ...
    axisBottom(gx.node() as SVGGElement, x0 as any)

    const gy = g.append("g")
    axisLeft(gy.node() as SVGGElement, y as any)
    ...
    groups
      ...
      .attr("rx", 10)
      .attr("fill", d => (d.key === "maternal" ? "rgba(20, 70, 160, 0.22)" : "rgba(160, 70, 20, 0.22)"))
      .attr("stroke", d => (d.key === "maternal" ? "rgba(20, 70, 160, 0.55)" : "rgba(160, 70, 20, 0.55)"))

    const legend = g.append("g").attr("transform", `translate(${innerW - 240}, 8)`)

    legend.append("rect")
      .attr("x", 0).attr("y", 0).attr("width", 16).attr("height", 16)
      .attr("fill", "rgba(20, 70, 160, 0.22)").attr("stroke", "rgba(20, 70, 160, 0.55)")

    legend.append("text")
      .attr("x", 24).attr("y", 12).attr("fontSize", 12)
      .text("Maternal health visits")

    legend.append("rect")
      .attr("x", 0).attr("y", 24).attr("width", 16).attr("height", 16)
      .attr("fill", "rgba(160, 70, 20, 0.22)").attr("stroke", "rgba(160, 70, 20, 0.55)")

    legend.append("text")
      .attr("x", 24).attr("y", 36).attr("fontSize", 12)
